...
vi.mock("../src/agents/agent-runner.js", async () => {
  const actual = await vi.importActual<typeof import("../src/agents/agent-runner.js")>("../src/agents/agent-runner.js");
...
describe("FleetView wiring (real extension lifecycle)", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(join(tmpDir, ".pi", "summoner.json"), JSON.stringify({ defaultJoinMode: "async" }));
