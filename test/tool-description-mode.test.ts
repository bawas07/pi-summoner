...
describe("toolDescriptionMode", () => {
  ...
  function setup(settings?: Record<string, unknown>, beforeInstantiate?: () => void) {
    ...
    // summoner.json can't leak into the "default is full" assertion.
    ...
    if (settings) {
      writeFileSync(join(tmpDir, ".pi", "summoner.json"), JSON.stringify(settings));
    }
    beforeInstantiate?.();
  ...
  it("defaults to the full description", () => {
    ...
    // Full agent descriptions are embedded (a late Scout sentence survives).
  ...
  it("compact mode swaps in the short description with one-line type list", () => {
    ...
    expect(desc).toContain("- Scout: Fast read-only search agent for locating code. (Tools:");
  ...
  it("the shipped example template renders byte-identical to the full description", async () => {
    ...
    writeFileSync(join(tmpDir, ".pi", "summoner.json"), JSON.stringify({ toolDescriptionMode: "full" }));
    const second = makePi();
