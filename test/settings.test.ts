 ...
 *   Simulates the user's project root. Settings live at `<projectDir>/.pi/summoner.json`.
 */
describe("settings persistence", () => {
  ...
  const globalFile = () => join(globalDir, "summoner.json");
  const projectFile = () => join(projectDir, ".pi", "summoner.json");
  ...
  describe("applyAndEmitLoaded", () => {
    ...
    it("loads, applies, and emits summoner:settings_loaded with merged settings", () => {
      writeGlobal({ maxConcurrent: 16 });
      ...
      expect(emit).toHaveBeenCalledWith("summoner:settings_loaded", {
    ...
    it("still emits the event when both files are missing (payload carries {})", () => {
      ...
      expect(emit).toHaveBeenCalledWith("summoner:settings_loaded", { settings: {} });
  ...
  describe("saveAndEmitChanged", () => {
    it("persists, emits with persisted=true, and returns info toast on success", () => {
      ...
      expect(emit).toHaveBeenCalledTimes(1);
      expect(emit).toHaveBeenCalledWith("summoner:settings_changed", {
        settings: snapshot,
        persisted: true,
      });
