...
// (foreground spawn, background spawn + get_subagent_result, a Scout spawn)
...
describe.runIf(LIVE)("subagents print-mode e2e (live LLM, opt-in)", () => {
  ...
  it(
    "Scout subagent_type — model dispatches a non-default agent type",
    async () => {
      run = await runPrintMode({
        prompt:
          "Use the Agent tool with subagent_type 'Scout' to look at the current working " +
      ...
      expect(
        calls.some((c) => String(c.subagent_type ?? "").toLowerCase() === "scout"),
      ).toBe(true);
    ...
    async () => {
      ...
      run = await runPrintMode({
        prompt: [
          ...
          "3) SCOUT: spawn a subagent with subagent_type 'Scout' to summarize the current",
      ...
      // — the Scout type was dispatched
      expect(calls.some((c) => String(c.subagent_type ?? "").toLowerCase() === "scout")).toBe(true);
