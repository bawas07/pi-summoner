...
describe("cross-extension RPC", () => {
  ...
  describe("ping RPC", () => {
    it("replies with protocol version", async () => {
      ...
      const reply = vi.fn();
      events.on("summoner:rpc:ping:reply:req-1", reply);
      events.emit("summoner:rpc:ping", { requestId: "req-1" });
    ...
    it("scopes replies — other requestIds do not receive it", async () => {
      ...
      events.on("summoner:rpc:ping:reply:req-other", wrongReply);
      events.emit("summoner:rpc:ping", { requestId: "req-1" });
  ...
  describe("spawn RPC", () => {
    it("returns agent id on success", async () => {
      registerRpcHandlers(deps);
      const reply = vi.fn();
      events.on("summoner:rpc:spawn:reply:req-s1", reply);
      events.emit("summoner:rpc:spawn", {
    ...
    it("passes options through to manager.spawn", async () => {
      registerRpcHandlers(deps);
      const reply = vi.fn();
      events.on("summoner:rpc:spawn:reply:req-s2", reply);
      events.emit("summoner:rpc:spawn", {
        requestId: "req-s2", type: "Scout", prompt: "find it",
        options: { description: "search", isBackground: true },
      });

      await vi.waitFor(() => expect(reply).toHaveBeenCalled());
      expect(manager.spawn).toHaveBeenCalledWith(
        deps.pi, ctx, "Scout", "find it",
    ...
    it("returns error when no active session", async () => {
      ...
      registerRpcHandlers(deps);
      const reply = vi.fn();
      events.on("summoner:rpc:spawn:reply:req-s3", reply);
      events.emit("summoner:rpc:spawn", {
    ...
    it("returns error when manager.spawn throws", async () => {
      ...
      registerRpcHandlers(deps);
      const reply = vi.fn();
      events.on("summoner:rpc:spawn:reply:req-s4", reply);
      events.emit("summoner:rpc:spawn", {
    ...
    it("scopes replies — other requestIds do not receive it", async () => {
      ...
      events.on("summoner:rpc:spawn:reply:req-other", wrongReply);
      events.on("summoner:rpc:spawn:reply:req-s5", rightReply);
      events.emit("summoner:rpc:spawn", {
    ...
    it("unsub stops responding to spawns", async () => {
      ...
      const reply = vi.fn();
      events.on("summoner:rpc:spawn:reply:req-s6", reply);
      events.emit("summoner:rpc:spawn", {
  ...
  describe("stop RPC", () => {
    it("returns success when agent is aborted", async () => {
      registerRpcHandlers(deps);
      const reply = vi.fn();
      events.on("summoner:rpc:stop:reply:req-st1", reply);
      events.emit("summoner:rpc:stop", { requestId: "req-st1", agentId: "agent-42" });
    ...
    it("returns error when agent not found", async () => {
      ...
      registerRpcHandlers(deps);
      const reply = vi.fn();
      events.on("summoner:rpc:stop:reply:req-st2", reply);
      events.emit("summoner:rpc:stop", { requestId: "req-st2", agentId: "nonexistent" });
    ...
    it("scopes replies — other requestIds do not receive it", async () => {
      ...
      const wrongReply = vi.fn();
      const rightReply = vi.fn();
      events.on("summoner:rpc:stop:reply:req-other", wrongReply);
      events.on("summoner:rpc:stop:reply:req-st3", rightReply);
      events.emit("summoner:rpc:stop", { requestId: "req-st3", agentId: "agent-42" });
    ...
    it("unsub stops responding to stop requests", async () => {
      ...
      const reply = vi.fn();
      events.on("summoner:rpc:stop:reply:req-st4", reply);
      events.emit("summoner:rpc:stop", { requestId: "req-st4", agentId: "agent-42" });
  ...
  describe("concurrent requests", () => {
    it("handles multiple simultaneous spawn requests independently", async () => {
      ...
      const reply1 = vi.fn();
      const reply2 = vi.fn();
      events.on("summoner:rpc:spawn:reply:req-a", reply1);
      events.on("summoner:rpc:spawn:reply:req-b", reply2);

      events.emit("summoner:rpc:spawn", { requestId: "req-a", type: "Scout", prompt: "first" });
      events.emit("summoner:rpc:spawn", { requestId: "req-b", type: "Crafter", prompt: "second" });
  ...
  describe("spawn RPC model override", () => {
    ...
    it("resolves a string model to a Model instance before manager.spawn", async () => {
      registerRpcHandlers(deps);
      const reply = vi.fn();
      events.on("summoner:rpc:spawn:reply:req-m1", reply);
      events.emit("summoner:rpc:spawn", {
    ...
    it("passes a Model object through unchanged", async () => {
      registerRpcHandlers(deps);
      const reply = vi.fn();
      events.on("summoner:rpc:spawn:reply:req-m2", reply);
      events.emit("summoner:rpc:spawn", {
    ...
    it("surfaces a clear error when the model string can't be resolved", async () => {
      registerRpcHandlers(deps);
      const reply = vi.fn();
      events.on("summoner:rpc:spawn:reply:req-m3", reply);
      events.emit("summoner:rpc:spawn", {
    ...
    it("errors when ctx has no modelRegistry but a string model is given", async () => {
      ...
      registerRpcHandlers(deps);
      const reply = vi.fn();
      events.on("summoner:rpc:spawn:reply:req-m4", reply);
      events.emit("summoner:rpc:spawn", {
