 ...
 * agent-summoner extension through REAL end-to-end subagent runs.
 *
 * WHY THIS EXISTS
 * ---------------
 ...
 * agent-summoner extension loaded, drives a real assistant turn that calls the
 ...
 *
 * It is the agent-summoner analogue of pi-chonky-step's `src/agent.ts` headless
 ...
 * background subagents complete (via the `Symbol.for("agent-summoner:manager")`
...
/** Path to the agent-summoner extension entrypoint (repo `src/index.ts`). */
