// Persistence for agent-summoner operational settings.
// - Global:  ~/.pi/agent/summoner.json (via getAgentDir()) — manual defaults, never written here
// - Project: <cwd>/.pi/summoner.json — written by /agents → Settings; overrides global on load
...
export interface SubagentsSettings {
  ...
  scopeModels?: boolean;
  /**
   * When true, the three built-in default agents (general-purpose, Scout, Plan)
  ...
  fleetView?: boolean;
  /**
   * When true (default), the extension operates in Plan mode: write/edit tools
...
function globalPath(): string {
  return join(getAgentDir(), "summoner.json");
}

function projectPath(cwd: string): string {
  return join(cwd, ".pi", "summoner.json");
}
...
function readSettingsFile(path: string): SubagentsSettings {
  ...
  try {
    ...
    console.warn(`[agent-summoner] Ignoring malformed settings at ${path}: ${reason}`);
    return {};
  }
}
 ...
 * `summoner:settings_loaded` lifecycle event. Returns the loaded settings so
...
export function applyAndEmitLoaded(
  ...
  emit("summoner:settings_loaded", { settings });
  return settings;
}

/**
 * Persist a settings snapshot, emit the `summoner:settings_changed` event
...
export function saveAndEmitChanged(
  ...
  emit("summoner:settings_changed", { settings: snapshot, persisted });
