...
import { loadPartyRules, resolvePartyModel } from "../party-rules.js";
 ...
 * Priority: explicit option > party.rules.json > config.model > parent model.
 */
function resolveDefaultModel(
  ...
  // party.rules.json model override takes priority over agent config and parent
  const effectiveModel = partyModel ?? configModel;
  if (effectiveModel) {
    const slashIdx = effectiveModel.indexOf("/");
    if (slashIdx !== -1) {
      const provider = effectiveModel.slice(0, slashIdx);
      const modelId = effectiveModel.slice(slashIdx + 1);
...
export interface RunOptions {
  ...
  pi: ExtensionAPI;
  /** Manager-assigned id; suffixes session name to disambiguate parallel spawns (e.g. `Scout#a1b2c3d4`). */
  agentId?: string;
  model?: Model<any>;
  maxTurns?: number;
  signal?: AbortSignal;
...
export async function runAgent(
  ...
  // Resolve model: explicit option > party.rules.json > config.model > parent model
  const partyRules = loadPartyRules(configCwd);
  const partyModel = resolvePartyModel(type, partyRules);
  const model = options.model ?? resolveDefaultModel(
    ctx.model, ctx.modelRegistry, agentConfig?.model, partyModel,
  );
